...
node {
    ...
    docker.withRegistry('http://10.100.192.200:5000') {
        ...
        def pcImg = docker.build("clickparadise/clickcount:${env.BUILD_TAG}")
...
node("prod") {
    stage name: 'PROD Deployment', concurrency: 1
    input message: "Does uat.clickparadise.fr/clickcount/ look good?"
    def nextColor = deploy("prod", "${prodMaster}", "${service}")

    stage name: 'PROD Post-Deployment', concurrency: 1
